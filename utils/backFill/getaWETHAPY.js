...
const getaWETHAPY = async (web3s, blockNum, timestamp) => {
     ...
     * Supply APY
     */
    const reserveData = await pool.methods.getReserveData(underlyingTokenAddress.WETH).call()
    ...
    let underlyingToken = new avax_web3.eth.Contract(ERC20Abi, underlyingTokenAddress.WETH)
    let aToken = new avax_web3.eth.Contract(ERC20Abi, avax_addresses.aWETH)
    let underlyingPriceFeed = new avax_web3.eth.Contract(priceFeedAbi, priceFeedAddresses.ETH)
    ...
    APYData.APY.description = "aWETH Supply + Distribution APY"
    APYData.APY.value = (incentiveDepositAPY + depositAPY) * (1 - fees.aWETH)
