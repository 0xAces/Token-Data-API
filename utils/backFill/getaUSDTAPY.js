...
const getaUSDTAPY = async (web3s, blockNum, timestamp) => {
     ...
     * Supply APY
     */
    const reserveData = await pool.methods.getReserveData(underlyingTokenAddress.USDT).call()
    ...
    let underlyingToken = new avax_web3.eth.Contract(ERC20Abi, underlyingTokenAddress.USDT)
    let aToken = new avax_web3.eth.Contract(ERC20Abi, avax_addresses.aUSDT)
    let underlyingPriceFeed = new avax_web3.eth.Contract(priceFeedAbi, priceFeedAddresses.USDT)
    ...
    APYData.APY.description = "aUSDT Supply + Distribution APY"
    APYData.APY.value = (incentiveDepositAPY + depositAPY) * (1 - fees.aUSDT)
